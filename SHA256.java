...
public class SHA256{
    ...
    private static int rightRotate(int val, int bits)
    {
    ...
    private static byte[] padMsg(byte[] msg) 
    {
        int len = msg.length;
        int newLen = len + 1;
        while ((newLen % 64) != 56) {
        ...
        byte[] padded = Arrays.copyOf(msg, newLen + 8);
    ...
    public static void main(String [] args) 
    {
        ...
        byte[] msg = padMsg(input.getBytes(StandardCharsets.UTF_8));
        int[] hashV = Arrays.copyOf(H, H.length);
        int[] words = new int[64];
        for (int i = 0; i < msg.length; i += 64) 
        {
            ByteBuffer buffer = ByteBuffer.wrap(msg, i, 64);
            for (int j = 0; j < 16; j++) 
            {
            ...
            int a = hashV[0], b = hashV[1], c = hashV[2], d = hashV[3];
            int e = hashV[4], f = hashV[5], g = hashV[6], h = hashV[7];
            for (int j = 0; j < 64; j++) 
            {
